...
{
    return (SCG_Mode_t)((SCG_CSR_SCS_MASK & SCG_CSR) >> SCG_CSR_SCS_SHIFT);
}
...
uint32_t ClockSetup(uint32_t opt)
{
    ...
    switch(opt)
    {
        case IRC_48M:
                ...
                SCG->SPLLCSR = 0;
                SCG->SPLLCFG = SCG_SPLLCFG_SOURCE(1) | SCG_SPLLCFG_MULT(0) | SCG_SPLLCFG_PREDIV(7);
        ...
        case IRC_96M:
                ...
                /* switch mode to SPLL */
                SCG->RCCR = SCG_RCCR_SCS(kSCG_PLL) | SCG_RCCR_DIVCORE(0) | SCG_RCCR_DIVSLOW(3);
                while(kSCG_PLL != SCG_GetMode());
