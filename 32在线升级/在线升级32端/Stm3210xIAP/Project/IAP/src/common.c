...
uint32_t GetIntegerInput(int32_t * num)
{
  ...
  while (1)
  {
    ...
    {
      SerialPutString("用户取消了 \r\n");
      return 0;
    }

    if (Str2Int(inputstr, num) == 0)
    {
      SerialPutString("错误，请重新输入: \r\n");
    }
    else
    {
      return 1;
...
void Main_Menu(void)
{
  ...
  {
    SerialPutString("\r\n================== Main Menu ============================\r\n\n");
    SerialPutString("  下载文件到 STM32F10x 内部 Flash ------- 1\r\n\n");
    SerialPutString("  上传文件到 STM32F10x 内部 Flash ------- 2\r\n\n");
    SerialPutString("  执行新的项目 ------------------------------ 3\r\n\n");
    
    if(FlashProtection != 0)
    {
      SerialPutString("  禁止写保护 ------------------------- 4\r\n\n");
    }
  ...
  //  key = GetKey();
   ...
   /** else if ((key == 0x34) && (FlashProtection == 1))
    ...
    else
    {
      if (FlashProtection == 0)
      {
        SerialPutString("无效数字 ! ==> 数字应该是 1, 2 或者 3\r");
      }
      else
      {
        SerialPutString("无效数字 ! ==> 数量应该是指 1, 2, 3 或 4\r");
      } 
    }*&***/
  }
}
